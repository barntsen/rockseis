...
template <typename T>
Derd<T>::Derd(const int _nx, const int _ny, const int _nz, const T _dx, const T _dy, const T _dz, const int _order)
{
    nx = _nx;
    ny = _ny;
    nz = _nz;
    ...
    lny = ny;
    lnz = nz;
    if((lnx*lny*lnz - 1) != ind(nx-1, ny-1, nz-1)) rs_error("Derd::Derd: The model size is beyond the size this program can model.");
...
template <typename T>
void Derd<T>::setOrder(const int _order)
{
    order=_order;
}
...
template class Derd<float>;
template class Derd<double>;
}
