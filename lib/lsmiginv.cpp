...
Lsmiginv<T>::Lsmiginv() {
    ...
    {
        rs_error("Lsmiginv<T>::Lsmiginv(): Error creating logfile for writting.");
    }
    if(createProglog() == INV_ERR)
    {
        rs_error("Lsmiginv<T>::Lsmiginv(): Error creating progress logfile for writting.");
    }
    noreverse = false;
    filter = false;
    zder = false;
...
template<typename T>
Lsmiginv<T>::Lsmiginv(MPImodeling *_mpi) {
    mpi = _mpi;
...
template<typename T>
void Lsmiginv<T>::normalize(double *v, double *f, int n){
	int i;
    if(fnorm == 0.0) {
        fnorm = 1.0;
    }
...
template<typename T>
double Lsmiginv<T>::vector_norm(double *v, const int type, const int n){
	// Variables
	int i;
	double norm;
...
template<typename T>
void Lsmiginv<T>::writeLog(std::string text){
    char tempo[256];
	time_t now;
    now = time(NULL);
...
template<typename T>
void Lsmiginv<T>::writeProgress(std::string text){
...
// =============== 2D ACOUSTIC LSMIGINV CLASS =============== //
//
template<typename T>
LsmiginvAcoustic2D<T>::LsmiginvAcoustic2D() {
    // Set default parameters
    apertx = -1;
...
template<typename T>
LsmiginvAcoustic2D<T>::LsmiginvAcoustic2D(MPImodeling *mpi): Lsmiginv<T>(mpi) {
    // Set default parameters
    apertx = -1;
...
template<typename T>
void LsmiginvAcoustic2D<T>::runGrad() {
	...
	if(mpi->getRank() == 0) {
        ...
        while(1) {
            ...
            if(work.MPItag == MPI_TAG_NO_WORK) {
                ...
                // Create lsrtm object
                lsrtm = std::make_shared<rockseis::LsrtmAcoustic2D<T>>(lmodel, lpimage, source, shot2Di, this->getOrder(), this->getSnapinc());
                ...
                // Hilbert transformed recorded data is stored in DataresVz
...
int LsmiginvAcoustic2D<T>::setInitial(double *x, std::string vpfile, std::string rhofile, std::string sourcefile)
{
    ...
    // Write linesearch image files
    std::shared_ptr<rockseis::Image2D<T>> pimage;
    pimage = std::make_shared<rockseis::Image2D<T>>(vpfile);
    if(!pimage->getAllocated()) pimage->allocateImage();
    pimage->setImagefile(VPLSFILE);
    pimage->write();
    ...
    switch(this->getParamtype()){
        ...
        default:
            N = (model_in->getGeom())->getNtot();
            rs_error("LsmiginvAcoustic2D<T>::setInitial(): Unknown parameterisation."); 
            break;
    }
...
template<typename T>
void LsmiginvAcoustic2D<T>::saveResults(int iter)
{
    std::string name;
    std::string dir;
    dir = RESULTDIR;
...
template<typename T>
void LsmiginvAcoustic2D<T>::saveLinesearch(double *x)
{
    // Models
    std::shared_ptr<rockseis::Image2D<T>> lsmodel;
    lsmodel = std::make_shared<rockseis::Image2D<T>>(VPLSFILE);
    std::shared_ptr<rockseis::ModelAcoustic2D<T>> mute;

    // Write linesearch model
    if(!lsmodel->getAllocated()) lsmodel->allocateImage();
    T *vpls;
    T *vpmutedata;
    vpls = lsmodel->getImagedata(); 
    int i;
    int N;
    ...
    if(!Mutefile.empty()){
        ...
        if(N != Nmute) rs_error("LsmiginvAcoustic2D<T>::saveLinesearch(): Geometry in Mutefile does not match geometry in the model.");
        mute->readModel();
...
template<typename T>
void LsmiginvAcoustic2D<T>::readMisfit(double *f)
{
    // Data misfit
    *f = 0.0;
    std::shared_ptr<rockseis::File> Fmisfit (new rockseis::File());
...
template<typename T>
void LsmiginvAcoustic2D<T>::readGrad(double *g)
{
    int i;
    int N;
    T *gvp;
    ...
    switch (this->getParamtype()){
        case PAR_GRID:
            ...
            break;
        default:
            rs_error("LsmiginvAcoustic2D<T>::readGrad(): Unknown parameterisation."); 
            break;
    }
}

template<typename T>
void LsmiginvAcoustic2D<T>::combineGradients()
{
    // Gradients
...
template<typename T>
void LsmiginvAcoustic2D<T>::applySrcilum()
{
    if(this->getSrcilum()){
        // Models
        std::shared_ptr<rockseis::ModelAcoustic2D<T>> model;
        model = std::make_shared<rockseis::ModelAcoustic2D<T>>(VPGRADFILE,VPGRADFILE, 1 ,0);
        // Ilumination
...
template<typename T>
void LsmiginvAcoustic2D<T>::applyMute()
{
    if(!Mutefile.empty()){
        // Models
        std::shared_ptr<rockseis::ModelAcoustic2D<T>> model;
...
template<typename T>
void LsmiginvAcoustic2D<T>::computeTikhonovRegularisation(double *x)
{
    // Models
...
template<typename T>
void LsmiginvAcoustic2D<T>::computeRegularisation(double *x)
{
    // Models
    std::shared_ptr<rockseis::ModelAcoustic2D<T>> model (new rockseis::ModelAcoustic2D<T>(VPLSFILE, VPLSFILE, 1 ,0));
    ...
    for(i=0; i< Nmod; i++)
    {
        M = dvpdx[i]*dvpdx[i] + dvpdz[i]*dvpdz[i] + reg_eps*reg_eps;
        M = sqrt(M);
        gwrk[i] = dvpdx[i]/M;
    }
    der->ddx_bw(gwrk);
...
// =============== 3D ACOUSTIC LSMIGINV CLASS =============== //
//
template<typename T>
LsmiginvAcoustic3D<T>::LsmiginvAcoustic3D() {
    // Set default parameters
    apertx = -1;
    aperty = -1;
...
template<typename T>
LsmiginvAcoustic3D<T>::LsmiginvAcoustic3D(MPImodeling *mpi): Lsmiginv<T>(mpi) {
    // Set default parameters
    apertx = -1;
    aperty = -1;
...
template<typename T>
void LsmiginvAcoustic3D<T>::runGrad() {
    MPImodeling *mpi = this->getMpi();
...
//            }
        ...
        /* Slave */
        std::shared_ptr<rockseis::LsrtmAcoustic3D<T>> lsrtm;
        while(1) {
            workModeling_t work = mpi->receiveWork();
            ...
            if(work.MPItag == MPI_TAG_NO_WORK) {
                ...
                // Create lsrtm object
                lsrtm = std::make_shared<rockseis::LsrtmAcoustic3D<T>>(lmodel, source, shot3Di, this->getOrder(), this->getSnapinc());
                ...
                if(dataweight){
                    ...
                    lsrtm->setDataweight(shotweight3Di);
                }
...
template<typename T>
void LsmiginvAcoustic3D<T>::runBsproj() {
    MPImodeling *mpi = this->getMpi();
	...
	if(vpproj==NULL){
		rs_error("LsmiginvAcoustic3D<T>::runBsproj(): Not enough memory to allocate projection array (vpproj)");
	}
	float *rhoproj= (float *) calloc(nc, sizeof(float));
	if(rhoproj==NULL){
		rs_error("LsmiginvAcoustic3D<T>::runBsproj(): Not enough memory to allocate projection array (rhoproj)");
	}
    ...
    if(mpi->getRank() == 0) {
		...
		if(global_stack==NULL){
			rs_error("LsmiginvAcoustic3D<T>::runBsproj(): Not enough memory to allocate global stack array");
		}
...
template<typename T>
int LsmiginvAcoustic3D<T>::setInitial(double *x, std::string vpfile, std::string rhofile, std::string sourcefile)
{
    ...
    switch(this->getParamtype()){
        ...
        case PAR_BSPLINE:
            ...
            break;
        default:
            rs_error("LsmiginvAcoustic3D<T>::setInitial(): Unknown parameterisation."); 
            break;
    }
...
template<typename T>
void LsmiginvAcoustic3D<T>::saveResults(int iter)
{
    std::string name;
    std::string dir;
    dir = RESULTDIR;
...
template<typename T>
void LsmiginvAcoustic3D<T>::saveLinesearch(double *x)
{
    // Models
    ...
    if(!Mutefile.empty()){
        ...
        int Nmute = (mute->getGeom())->getNtot();
        N = (lsmodel->getGeom())->getNtot();
        if(N != Nmute) rs_error("LsmiginvAcoustic3D<T>::saveLinesearch(): Geometry in Mutefile does not match geometry in the model.");
        mute->readModel();
        vpmutedata = mute->getVp();
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
            ...
            break;
        default:
            rs_error("LsmiginvAcoustic3D<T>::saveLinesearch(): Unknown parameterisation."); 
            break;
    }
    // Source wavelet
    Ns = lssource->getNt();
...
template<typename T>
void LsmiginvAcoustic3D<T>::readMisfit(double *f)
{
    // Data misfit
    *f = 0.0;
    std::shared_ptr<rockseis::File> Fmisfit (new rockseis::File());
...
template<typename T>
void LsmiginvAcoustic3D<T>::readGrad(double *g)
{
    int i;
    int N,Ns,Npar=0;
    float *g_in;
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
           spline = std::make_shared<rockseis::Bspl3D<T>>(modelgrad->getNx(), modelgrad->getNy(), modelgrad->getNz(), modelgrad->getDx(), modelgrad->getDy(), modelgrad->getDz(), this->getDtx(), this->getDty(), this->getDtz(), 3, 3, 3);
            ...
            break;
        default:
            rs_error("LsmiginvAcoustic3D<T>::readGrad(): Unknown parameterisation."); 
            break;
    }
    if(update_source){
        Ns = sourcegrad->getNt();
...
template<typename T>
void LsmiginvAcoustic3D<T>::combineGradients()
{
    // Gradients
...
//template<typename T>
//void LsmiginvAcoustic3D<T>::applySrcilum()
//{
//    if(this->getSrcilum()){
//        // Models
...
template<typename T>
void LsmiginvAcoustic3D<T>::applyMute()
{
    if(!Mutefile.empty()){
        // Models
        std::shared_ptr<rockseis::ModelAcoustic3D<T>> model;
...
template<typename T>
void LsmiginvAcoustic3D<T>::computeRegularisation(double *x)
{
    // Models
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
           ...
           spline = std::make_shared<rockseis::Bspl3D<double>>(model->getNx(), model->getNy(), model->getNz(), model->getDx(), model->getDy(), model->getDz(), this->getDtx(), this->getDty(), this->getDtz(), 3, 3, 3);
            ...
            break;
        default:
            rs_error("LsmiginvAcoustic3D<T>::computeRegularization(): Unknown parameterisation."); 
            break;
    }
    // Computing misfit
    double M; 
...
// =============== 2D ELASTIC LSMIGINV CLASS =============== //
//
template<typename T>
LsmiginvElastic2D<T>::LsmiginvElastic2D() {
    // Set default parameters
    apertx = -1;
    sourcetype = 0;
...
template<typename T>
LsmiginvElastic2D<T>::LsmiginvElastic2D(MPImodeling *mpi): Lsmiginv<T>(mpi) {
    // Set default parameters
    apertx = -1;
    sourcetype = 0;
...
template<typename T>
void LsmiginvElastic2D<T>::runGrad() {
    MPImodeling *mpi = this->getMpi();
	...
	if(mpi->getRank() == 0) {
        ...
        /* Slave */
        std::shared_ptr<rockseis::LsrtmElastic2D<T>> lsrtm;
        while(1) {
            workModeling_t work = mpi->receiveWork();
            ...
            if(work.MPItag == MPI_TAG_NO_WORK) {
                ...
                // Create lsrtm object
                lsrtm = std::make_shared<rockseis::LsrtmElastic2D<T>>(lmodel, source, Uxdata2Di, Uzdata2Di, this->getOrder(), this->getSnapinc());
                ...
                lsrtm->setDatamodUx(Uxdatamod2D);
                lsrtm->setDataresUx(Uxdatares2D);
                lsrtm->setDatamodUz(Uzdatamod2D);
                lsrtm->setDataresUz(Uzdatares2D);
                ...
                if(applyweightx){
                    ...
                    lsrtm->setDataweightx(xweight2Di);
                }
                if(applyweightz){
                    ...
                    lsrtm->setDataweightz(zweight2Di);
                }
                ...
                // Setting Snapshot file 
                lsrtm->setSnapfile(Snapfile + "-" + std::to_string(work.id));
                ...
                lsrtm->setNoreverse(this->getNoreverse());
...
template<typename T>
void LsmiginvElastic2D<T>::runBsproj() {
    MPImodeling *mpi = this->getMpi();
	T vpsum = 0.0; // Sum over splines
    ...
    if(vpproj==NULL){
        rs_error("LsmiginvElastic2D<T>::runBsproj(): Not enough memory to allocate projection array (vpproj)");
    }
    vsproj= (float *) calloc(nc, sizeof(float));
    if(vsproj==NULL){
        rs_error("LsmiginvElastic2D<T>::runBsproj(): Not enough memory to allocate projection array (vsproj)");
    }
    rhoproj= (float *) calloc(nc, sizeof(float));
    if(rhoproj==NULL){
        rs_error("LsmiginvElastic2D<T>::runBsproj(): Not enough memory to allocate projection array (rhoproj)");
    }
    ...
    if(mpi->getRank() == 0) {
		...
		global_stack= (float *) calloc(nc, sizeof(float));
		if(global_stack==NULL){
			rs_error("LsmiginvElastic2D<T>::runBsproj(): Not enough memory to allocate global stack array");
		}
...
template<typename T>
int LsmiginvElastic2D<T>::setInitial(double *x, std::string vpfile, std::string vsfile, std::string rhofile, std::string sourcefile)
{
    ...
    switch(this->getParamtype()){
        ...
        case PAR_BSPLINE:
            ...
            break;
        default:
            rs_error("LsmiginvElastic2D<T>::setInitial(): Unknown parameterisation."); 
            break;
    }
...
template<typename T>
void LsmiginvElastic2D<T>::saveResults(int iter)
{
    std::string name;
    std::string dir;
    dir = RESULTDIR;
...
template<typename T>
void LsmiginvElastic2D<T>::saveLinesearch(double *x)
{
    // Models
    ...
    if(!Mutefile.empty()){
        ...
        int Nmute = (mute->getGeom())->getNtot();
        N = (lsmodel->getGeom())->getNtot();
        if(N != Nmute) rs_error("LsmiginvElastic2D<T>::saveLinesearch(): Geometry in Mutefile does not match geometry in the model.");
        mute->readModel();
        vpmutedata = mute->getVp();
...
template<typename T>
void LsmiginvElastic2D<T>::readMisfit(double *f)
{
    // Data misfit
    *f = 0.0;
    std::shared_ptr<rockseis::File> Fmisfit (new rockseis::File());
...
template<typename T>
void LsmiginvElastic2D<T>::readGrad(double *g)
{
    int i;
    int N,Ns,Npar=0;
    float *g_in;
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
           spline = std::make_shared<rockseis::Bspl2D<T>>(modelgrad->getNx(), modelgrad->getNz(), modelgrad->getDx(), modelgrad->getDz(), this->getDtx(), this->getDtz(), 3, 3);
            ...
            break;
        default:
            rs_error("LsmiginvElastic2D<T>::readGrad(): Unknown parameterisation."); 
            break;
    }
    if(update_source){
        Ns = sourcegrad->getNt();
...
template<typename T>
void LsmiginvElastic2D<T>::combineGradients()
{
    // Gradients
...
template<typename T>
void LsmiginvElastic2D<T>::applyMute()
{
    if(!Mutefile.empty()){
        // Models
...
template<typename T>
void LsmiginvElastic2D<T>::computeRegularisation(double *x)
{
    // Models
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
            ...
            break;
        default:
            rs_error("LsmiginvElastic2D<T>::computeRegularization(): Unknown parameterisation."); 
            break;
    }
    // Computing misfit
    double M; 
...
// =============== 3D ELASTIC LSMIGINV CLASS =============== //
//
template<typename T>
LsmiginvElastic3D<T>::LsmiginvElastic3D() {
    // Set default parameters
    apertx = -1;
    aperty = -1;
    sourcetype = 0;
...
template<typename T>
LsmiginvElastic3D<T>::LsmiginvElastic3D(MPImodeling *mpi): Lsmiginv<T>(mpi) {
    // Set default parameters
    apertx = -1;
    aperty = -1;
    sourcetype = 0;
...
template<typename T>
void LsmiginvElastic3D<T>::runGrad() {
    MPImodeling *mpi = this->getMpi();
	...
	if(mpi->getRank() == 0) {
        ...
        /* Slave */
        std::shared_ptr<rockseis::LsrtmElastic3D<T>> lsrtm;
        while(1) {
            workModeling_t work = mpi->receiveWork();
            ...
            if(work.MPItag == MPI_TAG_NO_WORK) {
                ...
                // Create lsrtm object
                lsrtm = std::make_shared<rockseis::LsrtmElastic3D<T>>(lmodel, source, Uxdata3Di, Uydata3Di, Uzdata3Di, this->getOrder(), this->getSnapinc());
                ...
                lsrtm->setDatamodUx(Uxdatamod3D);
                lsrtm->setDataresUx(Uxdatares3D);
                lsrtm->setDatamodUy(Uydatamod3D);
                lsrtm->setDataresUy(Uydatares3D);
                lsrtm->setDatamodUz(Uzdatamod3D);
                lsrtm->setDataresUz(Uzdatares3D);
                ...
                if(applyweightx){
                    ...
                    lsrtm->setDataweightx(xweight3Di);
                }
                ...
                if(applyweighty){
                    ...
                    lsrtm->setDataweighty(yweight3Di);
                }
                if(applyweightz){
                    ...
                    lsrtm->setDataweightz(zweight3Di);
                }
...
template<typename T>
void LsmiginvElastic3D<T>::runBsproj() {
    MPImodeling *mpi = this->getMpi();
	T vpsum = 0.0; // Sum over splines
	T vssum = 0.0; // Sum over splines
	T rhosum = 0.0; // Sum over splines
    ...
    vpproj= (float *) calloc(nc, sizeof(float));
    if(vpproj==NULL){
        rs_error("LsmiginvElastic3D<T>::runBsproj(): Not enough memory to allocate projection array (vpproj)");
    }
    vsproj= (float *) calloc(nc, sizeof(float));
    if(vsproj==NULL){
        rs_error("LsmiginvElastic3D<T>::runBsproj(): Not enough memory to allocate projection array (vsproj)");
    }
    rhoproj= (float *) calloc(nc, sizeof(float));
    if(rhoproj==NULL){
        rs_error("LsmiginvElastic3D<T>::runBsproj(): Not enough memory to allocate projection array (rhoproj)");
    }
    ...
    if(mpi->getRank() == 0) {
		...
		global_stack= (float *) calloc(nc, sizeof(float));
		if(global_stack==NULL){
			rs_error("LsmiginvElastic3D<T>::runBsproj(): Not enough memory to allocate global stack array");
		}
...
template<typename T>
int LsmiginvElastic3D<T>::setInitial(double *x, std::string vpfile, std::string vsfile, std::string rhofile, std::string sourcefile)
{
    ...
    switch(this->getParamtype()){
        ...
        case PAR_BSPLINE:
            ...
            break;
        default:
            rs_error("LsmiginvElastic3D<T>::setInitial(): Unknown parameterisation."); 
            break;
    }
...
template<typename T>
void LsmiginvElastic3D<T>::saveResults(int iter)
{
    std::string name;
    std::string dir;
    dir = RESULTDIR;
...
template<typename T>
void LsmiginvElastic3D<T>::saveLinesearch(double *x)
{
    // Models
    ...
    if(!Mutefile.empty()){
        ...
        int Nmute = (mute->getGeom())->getNtot();
        N = (lsmodel->getGeom())->getNtot();
        if(N != Nmute) rs_error("LsmiginvElastic3D<T>::saveLinesearch(): Geometry in Mutefile does not match geometry in the model.");
        mute->readModel();
        vpmutedata = mute->getVp();
        vsmutedata = mute->getVs();
        rhomutedata = mute->getR();
...
template<typename T>
void LsmiginvElastic3D<T>::readMisfit(double *f)
{
    // Data misfit
    *f = 0.0;
    std::shared_ptr<rockseis::File> Fmisfit (new rockseis::File());
...
template<typename T>
void LsmiginvElastic3D<T>::readGrad(double *g)
{
    int i;
    int N,Ns,Npar=0;
    float *g_in;
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
           spline = std::make_shared<rockseis::Bspl3D<T>>(modelgrad->getNx(), modelgrad->getNy(), modelgrad->getNz(), modelgrad->getDx(), modelgrad->getDy(), modelgrad->getDz(), this->getDtx(), this->getDty(), this->getDtz(), 3, 3, 3);
            ...
            break;
        default:
            rs_error("LsmiginvElastic3D<T>::readGrad(): Unknown parameterisation."); 
            break;
    }
    if(update_source){
        Ns = sourcegrad->getNt();
...
template<typename T>
void LsmiginvElastic3D<T>::combineGradients()
{
    // Gradients
...
template<typename T>
void LsmiginvElastic3D<T>::applyMute()
{
    if(!Mutefile.empty()){
        // Models
...
template<typename T>
void LsmiginvElastic3D<T>::computeRegularisation(double *x)
{
    // Models
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
           ...
           spline = std::make_shared<rockseis::Bspl3D<double>>(model->getNx(), model->getNy(), model->getNz(), model->getDx(), model->getDy(), model->getDz(), this->getDtx(), this->getDty(), this->getDtz(), 3, 3, 3);
            ...
            break;
        default:
            rs_error("LsmiginvElastic3D<T>::computeRegularization(): Unknown parameterisation."); 
            break;
    }
    // Computing misfit
    double M; 
...
template class Lsmiginv<float>;
template class Lsmiginv<double>;

template class LsmiginvAcoustic2D<float>;
template class LsmiginvAcoustic2D<double>;

template class LsmiginvAcoustic3D<float>;
template class LsmiginvAcoustic3D<double>;

template class LsmiginvElastic2D<float>;
template class LsmiginvElastic2D<double>;

template class LsmiginvElastic3D<float>;
template class LsmiginvElastic3D<double>;
