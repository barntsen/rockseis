...
void WavesAcoustic2D<T>::forwardstepVelocity(std::shared_ptr<rockseis::ModelAcoustic2D<T>> model, std::shared_ptr<rockseis::Der<T>> der){
   int ix0, iz0, nx, nz, lpml, nxo, nzo;
   int ng = 6;
   int *Getapplypml = (int*)BallocNew(ng,sizeof(int));
...
void WavesAcoustic2D<T>::forwardstepStress(std::shared_ptr<rockseis::ModelAcoustic2D<T>> model, std::shared_ptr<rockseis::Der<T>> der){
   ...
   // Compute P and attenuate in x-direction 
   der->ddx_bw(Vx);

   Ac2dFwstepstressxc((float*)Pml->Vxx_left, (float*)Pml->Vxx_right, (float*)P,
              (float*)L,(float*)df,      
              (float*)Pml->A_ltf,(float*)Pml->B_ltf,(float *) Pml->C_ltf, 
              (float *)Pml->A_rbb,(float*)Pml->B_rbb,(float *)Pml->C_rbb, 
               Getapplypml,ix0,iz0,nxo,nzo,
              (float)dt,nx,nz,lpml,ng); 

   // Compute P and attenuate in z-direction
   der->ddz_bw(Vz);

   Ac2dFwstepstresszc((float*)Pml->Vzz_top, (float*)Pml->Vzz_bottom, 
