	...
	/*<Find nearest trace within the range specified by R>*/
{
	...
	presults = kd_nearest( tree, &pos[0]);
	nr=kd_res_size(presults);

	if(nr) {
		/* Fetch index of the nearest trace */
		pch =  kd_res_item( presults, tpos );
		dist=sqrt(SQ(tpos[0]-pos[0]) + SQ(tpos[1]-pos[1])+ SQ(tpos[2]-pos[2])+ SQ(tpos[3]-pos[3]));
	...
	/*<Find nearest trace within the range specified by R>*/
{
	...
	presults = kd_nearestf( tree, &pos[0]);
	nr=kd_res_size(presults);

	if(nr) {
		/* Fetch index of the nearest trace */
		pch =  kd_res_itemf( presults, tpos );
		dist=sqrt(SQ(tpos[0]-pos[0]) + SQ(tpos[1]-pos[1])+ SQ(tpos[2]-pos[2])+ SQ(tpos[3]-pos[3]));
	...
	/*<Find nearest trace within the range specified by R>*/
{
	...
	if(nr) {
		/* Fetch index of the nearest trace */
		pch =  kd_res_itemf( presults, tpos );
		dist=sqrt(SQ(tpos[0]-pos[0]) + SQ(tpos[1]-pos[1]));
