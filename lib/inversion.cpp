...
Inversion<T>::Inversion() {
   ...
   noreverse = false;
   filter = false;
   for (int i=0; i<3; i++) ndomain[i] = 1;
}

template<typename T>
Inversion<T>::Inversion(MPIdomaindecomp *_mpi) {
   mpi = _mpi;
   ...
   if(mpi->getRank() == 0) {
      ...
      while(1) {
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            lmodel = gmodel->getDomainmodel(shot2D, apertx, SMAP, mpi->getDomainrank(), this->getNdomain(0), this->getNdomain(1), this->getOrder());
            ...
            fwi->setSnapfile(Psnapfile + "-" + std::to_string(work.id) + "-" + std::to_string(mpi->getDomainrank()));
            ...
            shotmod2Di->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(2));
            Sort->put2DGather(shotmod2Di, work.id, (shotmod2Di->getGeom())->getGmap());
            ...
            shotres2Di->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(2));
            Sort->put2DGather(shotres2Di, work.id, (shotres2Di->getGeom())->getGmap());

            // Recompute and Output misfit
            shot2Di->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(2));
            shotmod2D->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(2));
            fwi->computeMisfit();
            T val = fwi->getMisfit();
            mpi->reduce(&val,1);
            if(mpi->getDomainrank() == 0){
            ...
            // Only write the part of the gradient that pertain to the domain
...
template<typename T>
InversionAcoustic3D<T>::InversionAcoustic3D(MPIdomaindecomp *mpi): Inversion<T>(mpi) {
   // Set default parameters
   apertx = -1;
   aperty = -1;
   ...
   if(mpi->getRank() == 0) {
      ...
      while(1) {
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            lmodel = gmodel->getDomainmodel(shot3D, apertx, aperty, SMAP, mpi->getDomainrank(), this->getNdomain(0), this->getNdomain(1), this->getNdomain(2), this->getOrder());
            (lmodel->getDomain())->setMpi(mpi);
            ...
            shotmod3Di->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(1),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(1),(lmodel->getDomain())->getPadh(2));
            Sort->put3DGather(shotmod3Di, work.id, (shotmod3Di->getGeom())->getGmap());
            ...
            shotres3Di->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(1),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(1),(lmodel->getDomain())->getPadh(2));
            Sort->put3DGather(shotres3Di, work.id, (shotres3Di->getGeom())->getGmap());

            // Recompute and Output misfit
            shot3Di->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(1),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(1),(lmodel->getDomain())->getPadh(2));
            shotmod3D->makeMap(lmodel->getGeom(),GMAP,(lmodel->getDomain())->getPadl(0),(lmodel->getDomain())->getPadl(1),(lmodel->getDomain())->getPadl(2),(lmodel->getDomain())->getPadh(0),(lmodel->getDomain())->getPadh(1),(lmodel->getDomain())->getPadh(2));
            fwi->computeMisfit();
            T val = fwi->getMisfit();
            mpi->reduce(&val,1);
            if(mpi->getDomainrank() == 0){
...
template<typename T>
InversionElastic2D<T>::InversionElastic2D(MPIdomaindecomp *mpi): Inversion<T>(mpi) {
   // Set default parameters
   apertx = -1;
   sourcetype = 0;
...
template<typename T>
InversionElastic3D<T>::InversionElastic3D(MPIdomaindecomp *mpi): Inversion<T>(mpi) {
   // Set default parameters
   apertx = -1;
   aperty = -1;
   sourcetype = 0;
...
// =============== 2D VISCOELASTIC INVERSION CLASS =============== //
//
template<typename T>
InversionViscoelastic2D<T>::InversionViscoelastic2D() {
   // Set default parameters
   apertx = -1;
   sourcetype = 0;
...
template<typename T>
InversionViscoelastic2D<T>::InversionViscoelastic2D(MPIdomaindecomp *mpi): Inversion<T>(mpi) {
   // Set default parameters
   apertx = -1;
   sourcetype = 0;
...
void InversionViscoelastic2D<T>::runGrad() {
   ...
   // Create a global model class
   std::shared_ptr<rockseis::ModelViscoelastic2D<T>> gmodel (new rockseis::ModelViscoelastic2D<T>(Vpfile, Vsfile, Rhofile, Qpfile, Qsfile, this->getF0(), this->getLpml() ,this->getFs()));
   ...
   if(mpi->getRank() == 0) {
      ...
      /* Slave */
      std::shared_ptr<rockseis::FwiViscoelastic2D<T>> fwi;
      while(1) {
         if(!mpi->ifActive()){
            break;
         }
...
void InversionViscoelastic2D<T>::runBsproj() {
   ...
   vpproj= (float *) calloc(nc, sizeof(float));
   if(vpproj==NULL){
      rs_error("InversionViscoelastic2D<T>::runBsproj(): Not enough memory to allocate projection array (vpproj)");
   }
   vsproj= (float *) calloc(nc, sizeof(float));
   if(vsproj==NULL){
   ...
   rhoproj= (float *) calloc(nc, sizeof(float));
   if(rhoproj==NULL){
      rs_error("InversionViscoelastic2D<T>::runBsproj(): Not enough memory to allocate projection array (rhoproj)");
   }
   ...
   if(mpi->getRank() == 0) {
      ...
      global_stack= (float *) calloc(nc, sizeof(float));
      if(global_stack==NULL){
         rs_error("InversionViscoelastic2D<T>::runBsproj(): Not enough memory to allocate global stack array");
      }
...
int InversionViscoelastic2D<T>::setInitial(double *x, std::string vpfile, std::string vsfile, std::string qpfile, std::string qsfile, std::string rhofile, std::string sourcefile)
{
   ...
   switch(this->getParamtype()){
      ...
      case PAR_BSPLINE:
         ...
         break;
      default:
         rs_error("InversionViscoelastic2D<T>::setInitial(): Unknown parameterisation."); 
         break;
   }
...
void InversionViscoelastic2D<T>::saveLinesearch(double *x)
{
   ...
   if(!Srcmutefile.empty()){
      ...
      Ns = sourcemute->getNt();
      if(Ns != source0->getNt()){
         rs_error("InversionViscoelastic2D<T>::saveLinesearch(): Geometry in Srcmutefile does not match geometry in the Source file.");
      }
   }
   ...
   if(!Modmutefile.empty()){
      ...
      int Nmute = (mute->getGeom())->getNtot();
      N = (lsmodel->getGeom())->getNtot();
      if(N != Nmute) rs_error("InversionViscoelastic2D<T>::saveLinesearch(): Geometry in Modmutefile does not match geometry in the model.");
      mute->readModel();
      vpmutedata = mute->getVp();
      vsmutedata = mute->getVs();
...
void InversionViscoelastic2D<T>::saveHessian(double *x)
{
   ...
   if(!Srcmutefile.empty()){
      ...
      Ns = sourcemute->getNt();
      if(Ns != lssource->getNt()){
         rs_error("InversionViscoelastic2D<T>::saveHessian(): Geometry in Srcmutefile does not match geometry in the Source file.");
      }
   }
   ...
   if(!Modmutefile.empty()){
      ...
      int Nmute = (mute->getGeom())->getNtot();
      N = (lsmodel->getGeom())->getNtot();
      if(N != Nmute) rs_error("InversionViscoelastic2D<T>::saveHessian(): Geometry in Modmutefile does not match geometry in the model.");
      mute->readModel();
      vpmutedata = mute->getVp();
      vsmutedata = mute->getVs();
      qpmutedata = mute->getVp();
   ...
   switch (this->getParamtype()){
      ...
      case PAR_BSPLINE:
         ...
         break;
      default:
         rs_error("InversionViscoelastic2D<T>::saveHessian(): Unknown parameterisation."); 
         break;
   }
...
void InversionViscoelastic2D<T>::readGrad(double *g)
{
   ...
   switch (this->getParamtype()){
      ...
      case PAR_BSPLINE:
         ...
         break;
      default:
         rs_error("InversionViscoelastic2D<T>::readGrad(): Unknown parameterisation."); 
         break;
   }
   if(update_source){
      Ns = sourcegrad->getNt();
...
void InversionViscoelastic2D<T>::computeRegularisation(double *x)
{
   ...
   switch (this->getParamtype()){
      ...
      case PAR_BSPLINE:
         ...
         break;
      default:
         rs_error("InversionViscoelastic2D<T>::computeRegularization(): Unknown parameterisation."); 
         break;
   }
   // Computing misfit
   double M; 
...
template class InversionViscoelastic2D<float>;
template class InversionViscoelastic2D<double>;
