...
template<typename T>
void KdmvaAcoustic2D<T>::runFatgrad() {
   ...
   if(mpi->getRank() == 0) {
      ...
      // Stack gradient
      vpgrad = std::make_shared<rockseis::Image2D<T>>(Fatvpgradfile, gmodel, 1, 1);
      vpgrad->createEmpty();

      for(long int i=0; i<ngathers; i++) {
         vpgrad->stackImage(Fatvpgradfile + "-" + std::to_string(i));
         remove_file(Fatvpgradfile + "-" + std::to_string(i));
      }
      ...
      while(1) {
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            vpgrad = std::make_shared<rockseis::Image2D<T>>(Fatvpgradfile + "-" + std::to_string(work.id), lmodel, 1, 1);
...
template<typename T>
void KdmvaAcoustic2D<T>::runMvagrad() {
   MPImodeling *mpi = this->getMpi();
   int nsoufin, nrecfin;
   size_t ngathers;
   ...
   if(mpi->getRank() == 0) {
      ...
      // -------------------------------------Compute gradient
      // Gradient
      vpgrad = std::make_shared<rockseis::Image2D<T>>(Mvavpgradfile, gmodel, 1, 1);
      vpgrad->createEmpty();
      ...
      for(long int i=0; i<ngathers; i++) {
         vpgrad->stackImage(Mvavpgradfile + "-" + std::to_string(i));
         remove_file(Mvavpgradfile + "-" + std::to_string(i));
      }
      ...
      while(1) {
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            vpgrad = std::make_shared<rockseis::Image2D<T>>(Mvavpgradfile + "-" + std::to_string(work.id), lmodel, 1, 1);
...
void KdmvaAcoustic2D<T>::combineGradients()
{
    ...
    std::shared_ptr<rockseis::ModelEikonal2D<T>> fatgrad;
    std::shared_ptr<rockseis::ModelEikonal2D<T>> mvagrad;
    std::shared_ptr<rockseis::ModelEikonal2D<T>> reggrad;
    fatgrad = std::make_shared<rockseis::ModelEikonal2D<T>>(FATVPGRADFILE, 1);
    mvagrad = std::make_shared<rockseis::ModelEikonal2D<T>>(MVAVPGRADFILE, 1);
    grad = std::make_shared<rockseis::ModelEikonal2D<T>>(FATVPGRADFILE, 1);
    reggrad = std::make_shared<rockseis::ModelEikonal2D<T>>(VPREGGRADFILE, 1);

    // Read gradients
    fatgrad->readVelocity();
    mvagrad->readVelocity();
    grad->readVelocity();
    reggrad->readVelocity();
    T *fatvp, *mvavp, *vp, *regvp;
    vp = grad->getVelocity(); 
    fatvp = fatgrad->getVelocity(); 
    mvavp = mvagrad->getVelocity(); 
    ...
    for(i=0; i< N; i++)
    {
        vp[i] = fatvp[i]*f_mva + mvavp[i]*f_fat + reg_alpha[0]*regvp[i];
    }
...
// =============== 3D ACOUSTIC KDMVA CLASS =============== //
//
template<typename T>
KdmvaAcoustic3D<T>::KdmvaAcoustic3D() {
    // Set default parameters
    apertx = -1;
    aperty = -1;
...
template<typename T>
KdmvaAcoustic3D<T>::KdmvaAcoustic3D(MPImodeling *mpi): Kdmva<T>(mpi) {
    // Set default parameters
    apertx = -1;
    aperty = -1;
...
template<typename T>
void KdmvaAcoustic3D<T>::runFatgrad() {
   MPImodeling *mpi = this->getMpi();
   ...
   if(mpi->getRank() == 0) {
      ...
      // Stack gradient
      vpgrad = std::make_shared<rockseis::Image3D<T>>(Fatvpgradfile, gmodel, 1, 1, 1);
      vpgrad->createEmpty();
      ...
      while(1) {
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            // Creating gradient objects
            vpgrad = std::make_shared<rockseis::Image3D<T>>(Fatvpgradfile + "-" + std::to_string(work.id), lmodel, 1, 1, 1);
            fat->setVpgrad(vpgrad);
...
template<typename T>
void KdmvaAcoustic3D<T>::runMvagrad() {
   MPImodeling *mpi = this->getMpi();
   int nsoufin, nrecfin;
   size_t ngathers;
   std::shared_ptr<rockseis::Data3D<T>> source;
   std::shared_ptr<rockseis::Data3D<T>> shot3D;
   std::shared_ptr<rockseis::RaysAcoustic3D<T>> rays;
   std::shared_ptr<rockseis::Ttable3D<T>> ttable;
   std::shared_ptr<rockseis::Image3D<T>> vpgrad;
   std::shared_ptr<rockseis::Image3D<T>> pimage;
   std::shared_ptr<rockseis::Image3D<T>> lpimage;
   std::shared_ptr<rockseis::ModelEikonal3D<T>> lmodel;
   ...
   if(mpi->getRank() == 0) {
      ...
      if(!this->getIncore()){
         ...
         // -------------------------------------Create a travel time table class
         ngathers = nsoufin + nrecfin;
         ttable = std::make_shared<rockseis::Ttable3D<T>> (gmodel, ngathers);
         ttable->setFilename(Ttablefile);
         ttable->createEmptyttable();
      ...
      // Image
      pimage = std::make_shared<rockseis::Image3D<T>>(Pimagefile, gmodel, this->getNhx(), this->getNhy(), this->getNhz());
      pimage->createEmpty();
      ...
      // -------------------------------------Compute gradient
      // Gradient
      vpgrad = std::make_shared<rockseis::Image3D<T>>(Mvavpgradfile, gmodel, 1, 1, 1);
      vpgrad->createEmpty();
      ...
      /* Slave */
      size_t number;
      std::shared_ptr<rockseis::Data3D<T>> Shotgeom;
      ...
      if(!this->getIncore()){
         ...
         while(1) {
            ...
            if(work.MPItag == MPI_TAG_NO_WORK) {
               ...
               // Set shot coordinates and make a map
               source = std::make_shared<rockseis::Data3D<T>>(1, 1, 1.0, 0.0);
               source->copyCoords(Shotgeom);

               source->makeMap(gmodel->getGeom(), SMAP);

               // Run modelling 
               rays = std::make_shared<rockseis::RaysAcoustic3D<T>>(gmodel);
               ...
               // Create traveltime table
               ttable = std::make_shared<rockseis::Ttable3D<T>> (Ttablefile);
               ttable->fetchTtabledata(rays, source, work.id); //Get traveltime data
               ttable->writeTtable(work.id);
               ttable.reset();
      ...
      // ------------------------------------Migrate data
      std::shared_ptr<rockseis::KdmigAcoustic3D<T>> kdmig;
      while(1) {
         workModeling_t work = mpi->receiveWork();
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            // Make image class
            pimage = std::make_shared<rockseis::Image3D<T>>(Pimagefile + "-" + std::to_string(work.id), lmodel, this->getNhx(), this->getNhy(), this->getNhz());
            ...
            // Create imaging class
            kdmig = std::make_shared<rockseis::KdmigAcoustic3D<T>>(lmodel, ttable, shot3D, pimage);
      ...
      while(1) {
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            // Make image class
            pimage = std::make_shared<rockseis::Image3D<T>>(PIMAGERESFILE);
            ...
            // Create imaging class
            kdmig = std::make_shared<rockseis::KdmigAcoustic3D<T>>(lmodel, ttable, shot3D, lpimage);
            ...
            // Creating gradient object
            vpgrad = std::make_shared<rockseis::Image3D<T>>(Mvavpgradfile + "-" + std::to_string(work.id), lmodel, 1, 1, 1);
...
template<typename T>
void KdmvaAcoustic3D<T>::runBsproj() {
   MPImodeling *mpi = this->getMpi();
   T vpsum = 0.0; // Sum over splines
   float *global_stack;
   T *c;
   ...
   float *vpproj= (float *) calloc(nc, sizeof(float));
   if(vpproj==NULL){
      rs_error("KdmvaAcoustic3D<T>::runBsproj(): Not enough memory to allocate projection array (vpproj)");
   }
   ...
   if(mpi->getRank() == 0) {
      ...
      global_stack= (float *) calloc(nc, sizeof(float));
      if(global_stack==NULL){
         rs_error("KdmvaAcoustic3D<T>::runBsproj(): Not enough memory to allocate global stack array");
      }
...
template<typename T>
int KdmvaAcoustic3D<T>::setInitial(double *x, std::string vpfile)
{
    ...
    if(this->getConstrain()){
       ...
       vp0 = model_in->getVelocity();
       for (long i=0; i< N; i++){
          if(lbounddata[i] == ubounddata[i]) rs_error("KdmvaAcoustic2D<T>::setInitial():The lower bound cannot be equal to the upper bound, use a mute function insted.");
    ...
    switch(this->getParamtype()){
        ...
        case PAR_BSPLINE:
            ...
            break;
        default:
            rs_error("KdmvaAcoustic3D<T>::setInitial(): Unknown parameterisation."); 
            break;
    }
...
template<typename T>
void KdmvaAcoustic3D<T>::saveResults(int iter)
{
    std::string name;
    std::string dir;
    dir = RESULTDIR;
...
template<typename T>
void KdmvaAcoustic3D<T>::saveLinesearch(double *x)
{
    // Models
    ...
    // If mute
    if(!Modelmutefile.empty()){
        mute = std::make_shared <rockseis::ModelEikonal3D<T>>(Modelmutefile, 1);
        long Nmute = (mute->getGeom())->getNtot();
        N = (lsmodel->getGeom())->getNtot();
        if(N != Nmute) rs_error("KdmvaAcoustic3D<T>::saveLinesearch(): Geometry in Modelmutefile does not match geometry in the model.");
        mute->readVelocity();
        vpmutedata = mute->getVelocity();
        N = (lsmodel->getGeom())->getNtot();
    }else{
    ...
    if(this->getConstrain()){
        ...
        long Nbound = (bound->getGeom())->getNtot();
        N = (lsmodel->getGeom())->getNtot();
        if(N != Nbound) rs_error("KdmvaAcoustic3D<T>::saveLinesearch(): Geometry in Boundary files does not match geometry in the model.");
        lbounddata = bound->getVp();
        ubounddata = bound->getR();
        x0 = (T *) calloc(N, sizeof(T));
    }
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
            ...
            break;
        default:
            rs_error("KdmvaAcoustic3D<T>::saveLinesearch(): Unknown parameterisation."); 
            break;
    }
    if(Modelmutefile.empty()){
        free(vpmutedata);
...
template<typename T>
void KdmvaAcoustic3D<T>::computeMisfit(std::shared_ptr<rockseis::Image3D<T>> pimage)
{
    T f=0;
    if(!pimage->getAllocated()) pimage->allocateImage();
       ...
       case SI:
          for (ihx=0; ihx<nhx; ihx++){
             ...
             for (ihy=0; ihy<nhy; ihy++){
                hy= -(nhy-1)/2 + ihy;
                G2 = G1*WEIGHT((T) hy, (T) ((nhy-1)/2));
                for (ihz=0; ihz<nhz; ihz++){
                   hz= -(nhz-1)/2 + ihz;
                   G3 = G2*WEIGHT((T) hz, (T) ((nhz-1)/2));
                   for (ix=0; ix<nx; ix++){
                      for (iy=0; iy<ny; iy++){
                         // Misfit
                         for (iz=0; iz<nz-1; iz++){
                            wrk[iz] = imagedata[ki3D(ix,iy,iz+1,ihx,ihy,ihz)] - imagedata[ki3D(ix,iy,iz,ihx,ihy,ihz)];
                         }
                         for (iz=0; iz<nz; iz++){
                            f -= 0.5*G2*wrk[iz]*wrk[iz];
                         }
                         // Residual
                         for (iz=1; iz<nz-1; iz++){
                            wrk[iz] = imagedata[ki3D(ix,iy,iz+1,ihx,ihy,ihz)] - 2.0*imagedata[ki3D(ix,iy,iz,ihx,ihy,ihz)] + imagedata[ki3D(ix,iy,iz-1,ihx,ihy,ihz)];
                         }
                         for (iz=1; iz<nz-1; iz++){
                            imagedata[ki3D(ix,iy,iz,ihx,ihy,ihz)] = G3*wrk[iz];
                         }
                         imagedata[ki3D(ix,iy,0,ihx,ihy,ihz)] = 0.0;
                         imagedata[ki3D(ix,iy,nz-1,ihx,ihy,ihz)] = 0.0;
                      }
                   }
                }
             }
             ...
             for (ihy=0; ihy<nhy; ihy++){
                for (ihx=0; ihx<nhx; ihx++){
                   for (iz=0; iz<nz; iz++){
                      for (iy=0; iy<ny; iy++){
                         for (ix=0; ix<nx; ix++){
                            imagedata[ki3D(ix,iy,iz,ihx,ihy,ihz)] *= mutedata[km3D(ix,iy,iz)];
                         }
                      }
                   }
                }
                ...
                for (ihz=0; ihz<nhz; ihz++){
                   ...
                   for (iy=0; iy<ny; iy++){
                      for (ix=0; ix<nx; ix++){
                         ...
                         for (iz=1; iz<nz-1; iz++){
                            imagedata[ki3D(ix,iy,iz,ihx,ihy,ihz)] = -1.0*G3*wrk[iz];
                         }
                         imagedata[ki3D(ix,iy,0,ihx,ihy,ihz)] = 0.0;
                         imagedata[ki3D(ix,iy,nz-1,ihx,ihy,ihz)] = 0.0;
                      }
                ...
                for (ihz=0; ihz<nhz; ihz++){
                   ...
                   for (iy=0; iy<ny; iy++){
                      for (ix=0; ix<nx; ix++){
                         ...
                         for (iz=0; iz<nz; iz++){
                            f1 += 0.5*((hx*hx) +  (hy*hy) + (hz*hz))*wrk[iz]*wrk[iz];
                            f2 += G3*wrk[iz]*wrk[iz];
                         }
                      }
                   }
                ...
                for (ihz=0; ihz<nhz; ihz++){
                   ...
                   for (iy=0; iy<ny; iy++){
                      for (ix=0; ix<nx; ix++){
                         ...
                         for (iz=1; iz<nz-1; iz++){
                            imagedata[ki3D(ix,iy,iz,ihx,ihy,ihz)] = (f1/(f2*f2))*G3*wrk[iz] - (((hx*hx) +  (hy*hy) +  (hz*hz))*1.0/f2)*wrk[iz]; 
                         }
                         imagedata[ki3D(ix,iy,0,ihx,ihy,ihz)] = 0.0;
                         imagedata[ki3D(ix,iy,nz-1,ihx,ihy,ihz)] = 0.0;
                      }
...
template<typename T>
void KdmvaAcoustic3D<T>::readMisfit(double *f)
{
    // Fat misfit
    *f = 0.0;
    f_fat = 0.0;
...
template<typename T>
void KdmvaAcoustic3D<T>::readGrad(double *g)
{
    int i;
    int N;
    float *g_in;
    ...
    switch (this->getParamtype()){
        ...
        case PAR_BSPLINE:
           spline = std::make_shared<rockseis::Bspl3D<T>>(modelgrad->getNx(), modelgrad->getNy(), modelgrad->getNz(), modelgrad->getDx(), modelgrad->getDy(), modelgrad->getDz(), this->getDtx(),this->getDty(), this->getDtz(), 3, 3, 3);
            ...
            break;
        default:
            rs_error("KdmvaAcoustic3D<T>::readGrad(): Unknown parameterisation."); 
            break;
    }
}

template<typename T>
void KdmvaAcoustic3D<T>::combineGradients()
{
    // Gradients
    std::shared_ptr<rockseis::ModelEikonal3D<T>> grad;
    std::shared_ptr<rockseis::ModelEikonal3D<T>> fatgrad;
    std::shared_ptr<rockseis::ModelEikonal3D<T>> mvagrad;
    std::shared_ptr<rockseis::ModelEikonal3D<T>> reggrad;
    fatgrad = std::make_shared<rockseis::ModelEikonal3D<T>>(FATVPGRADFILE, 1);
    mvagrad = std::make_shared<rockseis::ModelEikonal3D<T>>(MVAVPGRADFILE, 1);
    grad = std::make_shared<rockseis::ModelEikonal3D<T>>(FATVPGRADFILE, 1);
...
template<typename T>
void KdmvaAcoustic3D<T>::applyChainrule(double *x)
{
   // Models
   std::shared_ptr<rockseis::ModelEikonal3D<T>> model0 (new rockseis::ModelEikonal3D<T>(VP0FILE, 1));
   ...
   if(!Modelmutefile.empty()){
      ...
      long Nmute = (mute->getGeom())->getNtot();
      N = (grad->getGeom())->getNtot();
      if(N != Nmute) rs_error("KdmvaAcoustic3D<T>::applyChainrule(): Geometry in Modelmutefile does not match geometry in the model.");
      mute->readVelocity();
      vpmutedata = mute->getVelocity();
   }else{
      N = (grad->getGeom())->getNtot();
   ...
   long Nbound = (bound->getGeom())->getNtot();
   N = (grad->getGeom())->getNtot();
   if(N != Nbound) rs_error("KdmvaAcoustic3D<T>::applyChainrule(): Geometry in Boundary files does not match geometry in the model.");
   lbounddata = bound->getVp();
   ubounddata = bound->getR();
   x0 = (T *) calloc(N, sizeof(T));
   switch (this->getParamtype()){
      ...
      case PAR_BSPLINE:
         ...
         break;
      default:
         rs_error("KdmvaAcoustic3D<T>::applyChainrule(): Unknown parameterisation."); 
         break;
   }
   if(Modelmutefile.empty()){
      free(vpmutedata);
...
template<typename T>
void KdmvaAcoustic3D<T>::applyMute()
{
    if(!Modelmutefile.empty()){
        // Models
        ...
        // Mute
        std::shared_ptr<rockseis::ModelEikonal3D<T>> mute (new rockseis::ModelEikonal3D<T>(Modelmutefile, 1));
...
template<typename T>
void KdmvaAcoustic3D<T>::computeRegularisation(double *x)
{
    // Models
...
template class KdmvaAcoustic3D<float>;
template class KdmvaAcoustic3D<double>;
