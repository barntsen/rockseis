...
>*/
{
    ...
    for (i=0; i < ns; i++, buf += nb) {
      ...
      default: rockseis::rs_error("Unknown format"); break;
      }
    }
}
...
int main(int argc, char* argv[])
{
    ...
    if(esize_hdr > 4 || esize_data > 4) rockseis::rs_error("Double precision data not supported at the moment.");
	...
	if (SF_EBCBYTES != fwrite(ahead, 1, SF_EBCBYTES, file)) 
	   rockseis::rs_error("Error writing ebcdic header");
    ...
    if (SF_BNYBYTES != fwrite(bhead, 1, SF_BNYBYTES, file))
        rockseis::rs_error("Error writing binary header");
    format = segyformat (bhead);
    nsegy = SF_HDRBYTES + ((3 == format)? ns*2: ns*4);
    int *itrace = sf_intalloc (NTFILE); 
    itrace[19] = -SCALEL;
    itrace[20] = -SCALCO;
    ...
    switch(type){
        case rockseis::DATA2D:
            ...
            for (size_t i=0; i < ntr; i++)
            {
                ...
                itrace[12] = gcoords2D[0].y*SCALEL;
                itrace[71] = (int) (0.5*(itrace[21] + itrace[23]));

                dx = static_cast<double>(itrace[21]);
                dy = static_cast<double>(itrace[23]);
                doffset = sqrt(SQ(dx - dy))/SCALCO;
                itrace[11] = static_cast<int>(doffset);
                head2segy(trace, itrace, SF_NKEYS);
                trace2segy(trace + SF_HDRBYTES, ftrace, ns,format);
                if (nsegy != fwrite(trace, 1, nsegy, file))
                    rockseis::rs_error("Error writing traces");
            }
            Bdata2D->close();
            break;
        case rockseis::DATA3D:
            ...
            status = Bdata3D->open("r");
            if(status == FILE_ERR) rockseis::rs_error("Error opening file for reading");
            ...
            for (size_t i=0; i < ntr; i++)
            {
                ...
                itrace[23] = gcoords3D[0].x*SCALCO;
                itrace[24] = gcoords3D[0].y*SCALCO;
                itrace[12] = gcoords3D[0].z*SCALEL;
                itrace[71] = (int) (0.5*(itrace[21] + itrace[23]));
                itrace[72] = (int) (0.5*(itrace[22] + itrace[24]));
                dx = static_cast<double>(itrace[21] - itrace[23]);
                dy = static_cast<double>(itrace[22] - itrace[24]);
