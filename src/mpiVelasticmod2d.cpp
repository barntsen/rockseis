...
int main(int argc, char** argv) {
   ...
   std::shared_ptr<rockseis::ModelViscoelastic2D<float>> lmodel;
   ...
   std::shared_ptr<rockseis::ModelViscoelastic2D<float>> gmodel (new rockseis::ModelViscoelastic2D<float>(Vpfile, Vsfile, Rhofile, Qpfile, Qsfile, f0, lpml, fs));
   ...
   if(mpi.getRank() == 0) {
      ...
      std::shared_ptr<rockseis::ModellingViscoelastic2D<float>> modelling;
      while(1) {
         if(!mpi.ifActive()){
            break;
         }
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            modelling = std::make_shared<rockseis::ModellingViscoelastic2D<float>>(lmodel, source, order, snapinc);
