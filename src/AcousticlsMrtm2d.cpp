...
int main(int argc, char** argv) {
    ...
    if(argc < 2){
        if(mpi.getRank() == 0){
            ...
            PRINT_DOC(Mrecordfile = "Mshot.rss";);
    ...
    if(Inpar->getPar("Mrecordfile", &Mrecordfile) == INPARSE_ERR) status = true;
    ...
    lsmig->setMultiplefile(Mrecordfile);
