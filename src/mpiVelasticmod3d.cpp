...
int main(int argc, char** argv) {
    ...
    if(argc < 2){
        if(mpi.getRank() == 0){
            ...
            PRINT_DOC(            f0 = "20"; # Center frequency for calculation of relaxation times from Q models);
            ...
            PRINT_DOC(        Qp = "Qp2d.rss";);
            PRINT_DOC(        Qs = "Qs2d.rss";);
	...
	std::shared_ptr<rockseis::ModelViscoelastic3D<float>> lmodel;
    ...
    if(Inpar->getPar("Qp", &Qpfile) == INPARSE_ERR) status = true;
    if(Inpar->getPar("Qs", &Qsfile) == INPARSE_ERR) status = true;
	...
	std::shared_ptr<rockseis::ModelViscoelastic3D<float>> gmodel (new rockseis::ModelViscoelastic3D<float>(Vpfile, Vsfile, Rhofile, Qpfile, Qsfile, f0, lpml, fs));
	...
	if(mpi.getRank() == 0) {
        ...
        while(1) {
            ...
            if(work.MPItag == MPI_TAG_NO_WORK) {
                ...
                modelling = std::make_shared<rockseis::ModellingViscoelastic3D<float>>(lmodel, source, order, snapinc);
                ...
                if(Vxrecord){
                    ...
                    Sort->put3DGather(Vxdata3Di, work.id);
                }
                if(Vyrecord){
                    ...
                    Sort->put3DGather(Vydata3Di, work.id);
                }
                if(Vzrecord){
                    ...
                    Sort->put3DGather(Vzdata3Di, work.id);
                }
