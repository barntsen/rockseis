...
/* Global variables */
std::shared_ptr<TomoAcoustic3D<float>> fatt;
...
int main(int argc, char** argv) {
    ...
    if(argc < 2){
        if(mpi.getRank() == 0){
            PRINT_DOC(# MPI 3d acoustic traveltime inversion configuration file);
            PRINT_DOC();
    ...
    fatt = std::make_shared<rockseis::TomoAcoustic3D<float>>(&mpi);
