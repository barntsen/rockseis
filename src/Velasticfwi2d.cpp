...
/* Global variables */
std::shared_ptr<InversionViscoelastic2D<float>> inv;
...
int main(int argc, char** argv) {
   ...
   if(argc < 2){
      if(mpi.getRank() == 0){
         ...
         PRINT_DOC(f0 = "15"; # Center frequency for attenuation);
   ...
   inv = std::make_shared<rockseis::InversionViscoelastic2D<float>>(&mpi);
