   ...
   if(mpi.getRank() == 0) {
      ...
      while(1) {
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            Pdata3D = Sort->get3DGather(work.id);
            ...
            // Interpolate shot
            Pdata3Di = std::make_shared<rockseis::Data3D<float>>(ntr, source->getNt(), source->getDt(), 0.0);
            interp->interp(Pdata3D, Pdata3Di);
            Pdata3Di->setField(rockseis::PRESSURE);
            Pdata3Di->makeMap(lmodel->getGeom(), GMAP);
