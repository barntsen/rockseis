...
/* Global variables */
std::shared_ptr<KdmvaAcoustic2D<float>> kdmva;
...
int main(int argc, char** argv) {
   ...
   kdmva = std::make_shared<rockseis::KdmvaAcoustic2D<float>>(&mpi);
   ...
   kdmva->setTrecordfile(Trecordfile);
   kdmva->setDataweight(dataweight);
   kdmva->setDataweightfile(Dataweightfile);
   if(modelmute){
   ...
   kdmva->setFatmisfitfile(FATMISFITFILE);
   kdmva->setMvamisfitfile(MVAMISFITFILE);
   kdmva->setTmodelledfile(TMODFILE);
   kdmva->setTresidualfile(TRESFILE);
   ...
   kdmva->setFatvpgradfile(FATVPGRADFILE);
   kdmva->setMvavpgradfile(MVAVPGRADFILE);
