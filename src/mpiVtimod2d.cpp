...
int main(int argc, char** argv) {
   ...
   if(argc < 2){
      if(mpi.getRank() == 0){
         PRINT_DOC(# MPI 2d VTI modelling default configuration file);
         PRINT_DOC();
   ...
   if(mpi.getRank() == 0) {
      ...
      std::shared_ptr<rockseis::ModellingVti2D<float>> modelling;
      while(1) {
         if(!mpi.ifActive()){
            break;
         }
         ...
         if(work.MPItag == MPI_TAG_NO_WORK) {
            ...
            modelling = std::make_shared<rockseis::ModellingVti2D<float>>(lmodel, source, order, snapinc);
