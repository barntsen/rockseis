...
#define FATVPGRADFILE "fat_vp_grad.rss"
#define MVAVPGRADFILE "mva_vp_grad.rss"
...
#define FATMISFITFILE "fatmisfit.rss"
#define MVAMISFITFILE "mvamisfit.rss"
...
public:
    ...
    void setFatvpgradfile(std::string file) { Fatvpgradfile = file; }
    std::string getFatvpgradfile() { return Fatvpgradfile; }

    void setMvavpgradfile(std::string file) { Mvavpgradfile = file; }
    std::string getMvavpgradfile() { return Mvavpgradfile; }

    void setPimagefile(std::string file) { Pimagefile = file; }
    std::string getPimagefile() { return Pimagefile; }

    void setFatmisfitfile(std::string file) { Fatmisfitfile = file; }
    std::string getFatmisfitfile() { return Fatmisfitfile; }

    void setMvamisfitfile(std::string file) { Mvamisfitfile = file; }
    std::string getMvamisfitfile() { return Mvamisfitfile; }
    ...
    void runFatgrad();
    void runMvagrad();
    void runGrad() { this->runFatgrad(); this->runMvagrad(); }
...
private:
    ...
    std::string Fatvpgradfile;
    std::string Mvavpgradfile;
    std::string Pimagefile;
    std::string Fatmisfitfile;
    std::string Mvamisfitfile;
    std::string Snapfile;
...
// ##### ACOUSTIC 3D KDMVA CLASS
template<typename T>
class KdmvaAcoustic3D: public Kdmva<T> {
public:
    KdmvaAcoustic3D(); ///<Constructor
    KdmvaAcoustic3D(MPImodeling *_mpi); ///<Constructor
    ~KdmvaAcoustic3D(); ///<Destructor
    ...
    // Compute Misfit
    void computeMisfit(std::shared_ptr<rockseis::Image3D<T>> pimage);
