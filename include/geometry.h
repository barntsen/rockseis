...
public:
     ...
     * */
    void makeMap(std::shared_ptr<Geometry<T>> _geom, bool map, int padx, int pady); 
    void makeMap(std::shared_ptr<Geometry<T>> _geom, int padx, int pady) {makeMap(_geom,SMAP,padx,pady); makeMap(_geom,GMAP,padx,pady);} 
    void makeMap(std::shared_ptr<Geometry<T>> _geom) {makeMap(_geom,SMAP,0,0); makeMap(_geom,GMAP,0,0);} 
    void makeMap(std::shared_ptr<Geometry<T>> _geom, bool map) {makeMap(_geom,map,0,0);} 
    void copySmap2Gmap();
    void copyGmap2Smap();
